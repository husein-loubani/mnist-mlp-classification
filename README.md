...
**98.0% accuracy on the held-out test split**, from a two-layer network with about 235,000 parameters trained in under a minute on a laptop GPU.
...
| Tuned two-layer MLP | 97.7% |

The hidden layers are worth roughly six points over the best straight-line model in pixel space, which is the part of the result that the architecture actually earns.
...
| Batch size | Barely affects accuracy, strongly affects throughput; the smallest batch is the *slowest* |
